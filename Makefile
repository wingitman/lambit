...
.PHONY: all build build-all install install-shell uninstall clean test test-integration test-all cross-build
...
build-all:
	...
	GOOS=linux   GOARCH=amd64 go build -ldflags="-s -w -X github.com/wingitman/lambit/internal/version.Commit=$(COMMIT)" -o $(RELEASES_DIR)/linux/amd64/$(BINARY) .
	@echo "Building linux/arm64..."
	GOOS=linux   GOARCH=arm64 go build -ldflags="-s -w -X github.com/wingitman/lambit/internal/version.Commit=$(COMMIT)" -o $(RELEASES_DIR)/linux/arm64/$(BINARY) .
	@echo "Building darwin/amd64..."
	GOOS=darwin  GOARCH=amd64 go build -ldflags="-s -w -X github.com/wingitman/lambit/internal/version.Commit=$(COMMIT)" -o $(RELEASES_DIR)/darwin/amd64/$(BINARY) .
	@echo "Building darwin/arm64..."
	GOOS=darwin  GOARCH=arm64 go build -ldflags="-s -w -X github.com/wingitman/lambit/internal/version.Commit=$(COMMIT)" -o $(RELEASES_DIR)/darwin/arm64/$(BINARY) .
	@echo "Building windows/amd64..."
	GOOS=windows GOARCH=amd64 go build -ldflags="-s -w -X github.com/wingitman/lambit/internal/version.Commit=$(COMMIT)" -o $(RELEASES_DIR)/windows/$(BINARY).exe .
	@echo "Pre-built binaries written to $(RELEASES_DIR)/"

install:
	@mkdir -p $(INSTALL_DIR)
	@if command -v go >/dev/null 2>&1; then \
		echo "==> Go found - building lambit from source..."; \
		mkdir -p $(BUILD_DIR); \
		go build -ldflags="-s -w -X github.com/wingitman/lambit/internal/version.Commit=$(COMMIT)" -o $(BUILD_DIR)/$(BINARY) . || exit 1; \
